...
function makeMockCtx(width = 1920, height = 1080): CanvasRenderingContext2D {
  ...
  Object.defineProperty(ctx, 'globalAlpha', {
    ...
    set: (v: number) => { state.globalAlpha = v; },
    configurable: true,
  });
  return ctx;
}
...
describe('applyTransitionIn', () => {
  ...
  it('iris-open at progress=0.5 → no error (circle clip mid-point)', () => {
    const ctx = makeMockCtx();
    const renderFn = vi.fn();
    const config: TransitionConfig = { type: 'iris-open', duration: 0.5, easing: 'linear' };
    expect(() => {
  ...
  it('slide-left at progress=0 → renderFn is called', () => {
    const ctx = makeMockCtx();
    const renderFn = vi.fn();
    const config: TransitionConfig = { type: 'slide-left', duration: 0.5, easing: 'linear' };
  ...
  it('all transition types do not throw', () => {
    const types = ['none','fade','slide-left','slide-right','slide-up','slide-down','wipe-left','wipe-right','iris-open','iris-close','zoom-in','zoom-out','glitch'] as const;
    for (const type of types) {
      const ctx = makeMockCtx();
      const renderFn = vi.fn();
      const config: TransitionConfig = { type, duration: 0.5, easing: 'linear' };
      expect(() => {
...
describe('applyTransitionOut', () => {
  it('uses inverse progress of applyTransitionIn', () => {
    ...
    const xOut = (ctx1.translate as ReturnType<typeof vi.fn>).mock.calls[0][0];
    const xIn  = (ctx2.translate as ReturnType<typeof vi.fn>).mock.calls[0][0];
