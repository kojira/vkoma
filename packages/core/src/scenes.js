import { defineScene, fade, bounce, slide, zoom, params as sceneParams, } from "./base";
...
function drawTextWithEmoji(ctx, text, x, y, fontWeight, fontSize) {
...
const EqualizerScene = defineScene({
    ...
    draw: (ctx, rawParams, time) => {
        ...
        if (fftBandsRaw && typeof fftBandsRaw === "string" && fftBandsRaw.length > 0) {
            try {
                fftData = JSON.parse(fftBandsRaw);
            }
            catch { }
        }
        if (p.style === "bars") {
