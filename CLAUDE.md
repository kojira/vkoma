...
- **ユーザーの明示的な指示がない限り、フォールバック処理を実装しない**
- フォールバックは不具合を隠蔽し、原因特定を困難にする
- エラーが発生した場合は、握りつぶさずに適切なエラーメッセージを出力する（console.error、UIへの表示、例外のスローなど）
- `catch {}` で何もしない、デフォルト値で黙って代替する、等は禁止
- 正当な理由がある場合（renderCodeのparamsデフォルト値など）は例外とするが、その場合もコメントで理由を明記する
